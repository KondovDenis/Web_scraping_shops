...
class BrandysParser:
    ...
    def _get_category(self) -> list[str]:
        ...
        sheet = dataframe1['D2':'D55']
        sticker = dataframe1['B2':'B55']
    ...
    def get_items_from_category(self) -> list[str]:
        ...
        for val in category:
            ...
            for value in range(int(numberofpage)):
                print(value)
        ...
        for ref, sticker in self.items_ref.items():
            ...
            try:
                ...
                try:
                    ...
                    print(f'{ref} Error 140 NOT_FOUND_IMAGE2')
            try:
                ...
                size = 'NOT_FOUND_SIZE'
                print(f'{ref} Error 146 NOT_FOUND_SIZE')
            ...
            try:
                ...
                try:
                    ...
                    print(f'{ref} Error 157 NOT_FOUND_PRICE_OLD')
                    price_new = 'NOT_FOUND_PRICE_NEW'
                    print(f'{ref} Error 159 NOT_FOUND_PRICE_NEW')
            ...
            if size == 'NOT_FOUND_SIZE' or size == []:
                with open('..\\Parser\\BrandysClothes.csv', 'a', newline='', encoding='utf-8') as f:
                    ...
                    writer.writerows([[f'{title}', f'{title}', f'{articul}',f'Размер:{size}', f'Цвет:{color.strip()}', f'{image_1} {image_2}',f'{describe}', f'{price_old}', f'{price_new}', f'{sex}', f'{sticker}', f'{compound}'.replace('\n', '')]])
            elif size != 'NOT_FOUND_SIZE' or size != []:
                for sizes in size:
                    with open('..\\Parser\\BrandysClothes.csv', 'a', newline='', encoding='utf-8') as f:
                        writer = csv.writer(f, delimiter=';', escapechar=" ", quoting=csv.QUOTE_NONE, doublequote=True)
                        writer.writerows([[f'{title}', f'{title}', f'{articul}',f'Размер:{sizes}', f'Цвет:{color.strip()}', f'{image_1} {image_2}',f'{describe}', f'{price_old}', f'{price_new}', f'{sex}', f'{sticker}', f'{compound}'.replace('\n', '')]])
